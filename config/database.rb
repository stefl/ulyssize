...
case Padrino.env
  when :development then
    ...
    Mongoid.database = connection.db("silly-markov")
  when :production then
    uri  = URI.parse(ENV['MONGO_PRODUCTION_URI'] || ENV['MONGOLAB_URI'])
    connection = Mongo::Connection.from_uri(ENV['MONGO_PRODUCTION_URI'] || ENV['MONGOLAB_URI'], {:pool_timeout => 60, :pool_size => 5, :timeout => 10})
